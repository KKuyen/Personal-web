...
import styles from './Skill_tab.module.css';
import Summary from '../context/Skills_context';
import Sun from '../context/Saturn_context';
...
const Function5: React.FC<Function2Props> = ({ setActiveLink }) => {
  ...
  const changeBackground = () => {
    ...
    if (element) {
      ...
      if (isFirstColor) {
        ...
        if (sunElement) {
          sunElement.animate([
            { transform: 'translateX(-60%)', height: '70vh', width: '330vh' },
            { transform: 'translateX(-90%)', height: '30vh', width: '65vh' }
          ], {
      ...
      if (!isFirstColor) {
        ...
        if (sunElement) {
          sunElement.animate([
            ...
            { transform: 'translateX(-88%)', height: '30vh', width: '65vh'},
            { transform: 'translateX(-60%)', height: '70vh', width: '330vh' }
          ], {
            duration: 600, // 0.5 giây
            fill: 'forwards' // Giữ trạng thái cuối cùng của animation
          });
        }
        back.animate([
